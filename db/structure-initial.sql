...
CREATE VIEW project_totals AS
 SELECT backers.project_id,
    ...
    ((sum(backers.value) / projects.goal) * (100)::numeric) AS progress,
...
CREATE VIEW project_financials AS
 ...
 SELECT p.id AS project_id,
    ...
    ((catarse_base_url.value || '/admin/reports/backer_reports.csv?project_id='::text) || p.id) AS backer_report,
    p.state
   FROM ((((projects p
   JOIN users u ON ((u.id = p.user_id)))
...
--
-- Name: index_notifications_on_backer_id_and_template_name; Type: INDEX; Schema: public; Owner: -; Tablespace:
--

CREATE INDEX index_notifications_on_backer_id_and_template_name ON notifications USING btree (backer_id, template_name);
